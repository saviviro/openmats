...
Five worktree-isolated Codex tasks provide several chances for a due check to
start while the computer and Codex app are running. Three weekly triggers share
one routine check interval of 168 hours. Two monthly triggers share one
broad-discovery interval of 28 days. A trigger is only an opportunity to start:
it does not repeat a check that is not yet due. The user's active local branch
and unfinished local work do not affect these worktree runs.
